    ...
    class PasteTransform implements TextTransform {
        ...
        public void invokeOn(StringBuilder text, int startIndex, int endIndex) {
            if (startIndex == endIndex) {
                String lastChar  = String.valueOf(builder.charAt(builder.length() - 1)).toUpperCase();
                text.insert(startIndex, builder.substring(0, builder.length() - 1) + lastChar);
            } else {
                text.replace(startIndex, endIndex, builder.toString());
            }
        }
    }
